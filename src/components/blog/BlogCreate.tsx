...
export default function BlogCreate({ onSaved }: { onSaved: (d: any) => void }) {
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState("");
  const save = async (d: any) => { setSaving(true); setErr(""); try { await onSaved(d); } catch (e: any) { setErr(e.message || "失败"); } finally { setSaving(false); } };
  const el = <div className="max-w-3xl mx-auto px-4 py-8">
    <h1 className="font-sans text-display text-ink-primary mb-4">写新文章</h1>
    ...
    <Editor mode="blog" onSaved={save} onCancel={() => { window.location.href = "/blog"; }} />
  </div>;
  return el;
}
