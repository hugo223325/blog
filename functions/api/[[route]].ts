...
app.get("/api/posts", async (c) => {
  ...
  return c.json((results || []).map((r: any) => ({ ...r, tags: JSON.parse(r.tags || "[]") })));
});
...
app.post("/api/diary", requireAuth, async (c) => {
  ...
  const now = new Date().toISOString();
  await c.env.DB.prepare(
    "INSERT INTO diary (date, content, mood, tags, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(date) DO UPDATE SET content=excluded.content, mood=excluded.mood, tags=excluded.tags, updated_at=excluded.updated_at"
...
app.post("/api/todos", requireAuth, async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO todos (id, text, done, priority, created_at, due_date) VALUES (?, ?, ?, ?, ?, ?)").bind(id, body.text, body.done ? 1 : 0, body.priority || "medium", now, body.dueDate || null).run();
...
app.post("/api/schedule", requireAuth, async (c) => {
  ...
  const now = new Date().toISOString();
  const id = body.id || crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO schedule (id, title, date, time, duration, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, body.title, body.date, body.time || "09:00", body.duration || 60, body.notes || "", now).run();
  return c.json({ id, ok: true });
});
...
app.post("/api/weight", requireAuth, async (c) => {
  ...
  const now = new Date().toISOString();
  const id = body.id || crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO weight (id, date, weight, note, created_at) VALUES (?, ?, ?, ?, ?)").bind(id, body.date, body.weight, body.note || "", now).run();
  return c.json({ id, ok: true });
});
...
app.get("/api/settings", async (c) => {
  ...
  for (const r of results || []) { obj[(r as any).key] = (r as any).value; }
  return c.json(obj);
});
...
app.put("/api/settings/password", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO settings (key, value) VALUES ('password', ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(body.newPassword).run();
  return c.json({ ok: true });
});
